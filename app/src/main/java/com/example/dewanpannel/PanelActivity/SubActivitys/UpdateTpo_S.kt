...
import com.example.dewanpannel.databinding.ActivityUpdateTpoSBinding
...
class UpdateTpo_S : AppCompatActivity() {
    private lateinit var binding:ActivityUpdateTpoSBinding
    private lateinit var auth: FirebaseAuth
    private lateinit var databaseReference: DatabaseReference
    private lateinit var storageReference: StorageReference
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding=ActivityUpdateTpoSBinding.inflate(layoutInflater)
        setContentView(binding.root)
        auth = FirebaseAuth.getInstance()
        databaseReference = FirebaseDatabase.getInstance().getReference("Updates Tpo to Students")
        storageReference = FirebaseStorage.getInstance().getReference("Updates Tpo to Students")
                    ...
                    databaseReference.child("Information").setValue(user)
                        .addOnCompleteListener({
                            ...
                            if (it.isSuccessful) {
                                ...
                                var notification=Notification.Builder(this, Notificationcass().Channel2)
                                notification.setContentTitle("Tpo hai")
                                notification.setSmallIcon(R.drawable.boss)
                                notification.setContentText("kha ho aap")
                                    .build()
